...
public enum KeyPressParser {
    public static func parse(_ specification: String) throws -> ParsedKeyPress {
        ...
        if trimmed == " " || trimmed.isEmpty && specification.contains(" ") {
            return ParsedKeyPress(keyCode: CGKeyCode(kVK_Space), modifiers: [], displayValue: "space")
        }
        ...
        guard let keyToken = tokens.last else {
            throw ComputerUseError.invalidArguments(
                "key specification is empty. " +
                "Examples: \"Return\", \"Tab\", \"space\", \"super+c\", \"shift+a\". " +
                "For space bar use \"space\", for enter use \"Return\"."
            )
        }

        let modifiers = try tokens.dropLast().map(parseModifier(_:))
        guard let keyCode = keyCodeMap[keyToken] else {
            let suggestions = keyCodeMap.keys.sorted()
                .filter { $0.hasPrefix(keyToken.prefix(1)) }
                .prefix(5)
                .joined(separator: ", ")
            let hint = suggestions.isEmpty ? "" : " Did you mean: \(suggestions)?"
            throw ComputerUseError.invalidArguments(
                "unsupported key '\(specification)'.\(hint) " +
                "Common keys: Return, Tab, space, Escape, Up, Down, Left, Right, backspace, super+c."
            )
        }

        return ParsedKeyPress(keyCode: keyCode, modifiers: modifiers, displayValue: keyToken)
    }
    ...
    private static let keyCodeMap: [String: CGKeyCode] = [
        ...
        "space": CGKeyCode(kVK_Space), "spacebar": CGKeyCode(kVK_Space),
        "escape": CGKeyCode(kVK_Escape), "esc": CGKeyCode(kVK_Escape),
        "backspace": CGKeyCode(kVK_Delete), "delete": CGKeyCode(kVK_Delete),
        "del": CGKeyCode(kVK_ForwardDelete), "forwarddelete": CGKeyCode(kVK_ForwardDelete),
        ...
        "pageup": CGKeyCode(kVK_PageUp), "page_up": CGKeyCode(kVK_PageUp), "prior": CGKeyCode(kVK_PageUp),
        "pagedown": CGKeyCode(kVK_PageDown), "page_down": CGKeyCode(kVK_PageDown), "next": CGKeyCode(kVK_PageDown),
        ...
        "kp_0": CGKeyCode(kVK_ANSI_Keypad0), "kp_1": CGKeyCode(kVK_ANSI_Keypad1),
        "kp_2": CGKeyCode(kVK_ANSI_Keypad2), "kp_3": CGKeyCode(kVK_ANSI_Keypad3),
        "kp_4": CGKeyCode(kVK_ANSI_Keypad4), "kp_5": CGKeyCode(kVK_ANSI_Keypad5),
        "kp_6": CGKeyCode(kVK_ANSI_Keypad6), "kp_7": CGKeyCode(kVK_ANSI_Keypad7),
        "kp_8": CGKeyCode(kVK_ANSI_Keypad8), "kp_9": CGKeyCode(kVK_ANSI_Keypad9),
