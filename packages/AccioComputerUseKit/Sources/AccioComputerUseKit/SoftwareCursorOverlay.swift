...
func visualCursorEnabled(environment: [String: String]) -> Bool {
    for name in ["ACCIO_COMPUTER_USE_VISUAL_CURSOR", "OPEN_COMPUTER_USE_VISUAL_CURSOR"] {
...
public func visualCursorObservationFileURL(environment: [String: String]) -> URL? {
    for name in ["ACCIO_COMPUTER_USE_VISUAL_CURSOR_OBSERVATION_FILE", "OPEN_COMPUTER_USE_VISUAL_CURSOR_OBSERVATION_FILE"] {
        guard
            let rawPath = environment[name]?.trimmingCharacters(in: .whitespacesAndNewlines),
            !rawPath.isEmpty
        else {
            continue
        }
...
public let openComputerUseTurnEndedNotificationName = Notification.Name("com.accio.computeruse.turn-ended")
...
enum SoftwareCursorOverlay {
    ...
    private static func prepareWindowIfNeeded() {
        ...
        panel.title = "Accio Software Cursor"
    ...
    private static func configureOrdering(relativeTo targetWindow: CursorTargetWindow?, forceReorder: Bool) {
        ...
        // Place cursor at the same window level as the target app window.
        ...
        if shouldReorderCursorPanel(
            ...
            if let effectiveTargetWindow {
                // Order just above the target app's window — same z-level.
                panel.order(.above, relativeTo: Int(effectiveTargetWindow.windowID))
            } else {
                // No target window: order at front of current level (not above all apps).
